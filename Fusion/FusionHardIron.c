...
static FUSION_INLINE void AddSample(FusionHardIron *const hardIron, const FusionVector sample);
...
static bool IsNan(const Matrix *const m);
...
void FusionHardIronSetSettings(FusionHardIron *const hardIron, const FusionHardIronSettings *const settings) {
    FusionHardIronAbort(hardIron); // TODO: fix UB when reading hardIron->status
...
FusionResult FusionHardIronSolve(const FusionVector *const samples, const int numberOfSamples, FusionVector *const offset) {
    ...
    if (IsNan(theta)) {
        result = FusionResultSingularMatrix;
        goto cleanup;
    }
...
/**
 * @brief Returns true if any matrix element is NaN.
 * @param m Matrix.
 * @return True if any matrix element is NaN.
 */
static bool IsNan(const Matrix *const m) {
    for (int index = 0; index < (m->rows * m->cols); index++) {
        if (isnan(m->array[index]) != 0) {
            return true;
        }
    }
    return false;
