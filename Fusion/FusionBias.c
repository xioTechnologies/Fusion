...
void FusionBiasSetSettings(FusionBias *const bias, const FusionBiasSettings *const settings) {
    FusionBiasAbort(bias); // TODO: fix UB when reading bias->status

    bias->filterCoefficient = 2.0f * (float) M_PI * FILTER_CUTOFF * (1.0f / settings->sampleRate);
...
FusionProgress FusionBiasGetProgress(const FusionBias *const bias) {
    const uint32_t duration = bias->duration > 0 ? bias->duration : 1;
    const unsigned int percentage = (unsigned int) ((100 * bias->durationTimer) / duration);
