...
static PyObject *np_array_nx3_from(const float *const c_array, const int n) {
    const npy_intp dimensions[] = {n, 3};
    PyObject *np_array = PyArray_SimpleNew(2, dimensions, NPY_FLOAT);
    ...
    memcpy(PyArray_DATA((PyArrayObject *) np_array), c_array, n * 3 * sizeof(float));
    return np_array;
}
