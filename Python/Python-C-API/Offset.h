...
typedef struct {
  PyObject_HEAD FusionOffset offset;
} Offset;
...
static void offset_free(Offset *self) { Py_TYPE(self)->tp_free(self); }
...
static PyObject *offset_update_batch(Offset *self, PyObject *args) {
  ...
  // Convert to float32 C-contiguous array (handles float64, int, F-contiguous, etc.)
  input_array =
      (PyArrayObject *)PyArray_FromAny((PyObject *)input_array, PyArray_DescrFromType(NPY_FLOAT), 2,
                                       2, NPY_ARRAY_C_CONTIGUOUS | NPY_ARRAY_FORCECAST, NULL);
  if (!input_array)
    return NULL;
...
static PyTypeObject offset_object = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "imufusion.Offset",
