...
typedef struct {
  PyObject_HEAD FusionQuaternion quaternion;
} Quaternion;
...
static void quaternion_free(Quaternion *self) { Py_TYPE(self)->tp_free(self); }
...
static PyTypeObject quaternion_object = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "imufusion.Quaternion",
