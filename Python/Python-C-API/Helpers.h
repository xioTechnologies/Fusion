...
#define PARSE_TUPLE(args, format, ...)                                                             \
  (PyArg_ParseTuple(args, format, __VA_ARGS__) == 0 ? create_parse_tuple_error_string(format)      \
                                                    : NULL)

static void concatenate(char *const destination, const size_t destination_size,
...
static char *const parse_array(float *const destination, const PyArrayObject *const array,
                               const int size) {
