...
typedef struct {
  PyObject_HEAD FusionAhrsSettings settings;
} Settings;
...
static PyObject *settings_new(PyTypeObject *subtype, PyObject *args, PyObject *keywords) {
  ...
  const char *const error =
      PARSE_TUPLE(args, "iffffI", &self->settings.convention, &self->settings.gain,
                  &self->settings.gyroscopeRange, &self->settings.accelerationRejection,
                  &self->settings.magneticRejection, &self->settings.recoveryTriggerPeriod);
  if (error != NULL) {
...
static void settings_free(Settings *self) { Py_TYPE(self)->tp_free(self); }
...
static PyGetSetDef settings_get_set[] = {
    ...
    {"gyroscope_range", (getter)settings_get_gyroscope_range, (setter)settings_set_gyroscope_range,
     "", NULL},
    {"acceleration_rejection", (getter)settings_get_acceleration_rejection,
     (setter)settings_set_acceleration_rejection, "", NULL},
    {"magnetic_rejection", (getter)settings_get_magnetic_rejection,
     (setter)settings_set_magnetic_rejection, "", NULL},
    {"recovery_trigger_period", (getter)settings_get_recovery_trigger_period,
     (setter)settings_set_recovery_trigger_period, "", NULL},
    {NULL} /* sentinel */
};

static PyTypeObject settings_object = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "imufusion.Settings",
