...
| `accelerationRecoveryTrigger` | Acceleration recovery trigger value between 0.0 and 1.0. Acceleration recovery will activate when this value reaches 1.0 and then deactivate when the value reaches 0.0.                                                                                                                                |
...
| `magneticRecoveryTrigger`     | Magnetic recovery trigger value between 0.0 and 1.0. Magnetic recovery will activate when this value reaches 1.0 and then deactivate when the value reaches 0.0.                                                                                                                                        |
...
The `FUSION_INLINE` macro implements either `__forceinline` or `inline __attribute__((always_inline))` to inline functions and optimise for speed at the cost of program size. This can be disabled by uncommenting the `FUSION_NO_INLINE` in [FusionConfig.h](https://github.com/xioTechnologies/Fusion/blob/main/Fusion/FusionConfig.h) or adding this as a preprocessor definition.

### Fast inverse square root

Vector and quaternion normalisation operations use [Pizer's implementation](https://pizer.wordpress.com/2008/10/12/fast-inverse-square-root/) of the [fast inverse square root](https://en.wikipedia.org/wiki/Fast_inverse_square_root) algorithm. This increases speed for devices without an FPU, at a negligible cost to accuracy. Uncommenting `FUSION_USE_NORMAL_SQRT` in [FusionConfig.h](https://github.com/xioTechnologies/Fusion/blob/main/Fusion/FusionConfig.h) or adding this as a preprocessor definition will use normal square root for all operations.
